...
object ScalanAst {
  ...
  case class SMethodDef(name: String, tpeArgs: STpeArgs, argSections: List[SMethodArgs],
                        tpeRes: Option[STpeExpr], isImplicit: Boolean, overloadId: Option[String],
                        external: Option[MethodAnnotation], sql: Option[String] = None, elem: Option[Unit] = None) extends SBodyItem {
  ...
  object SEntityModuleDef {
    ...
    def apply(packageName: String, imports: List[SImportStat], moduleTrait: STraitDef, config: CodegenConfig, sql: String): SEntityModuleDef = {
      ...
      SEntityModuleDef(packageName, imports, moduleName, entityRepSynonym, entity, traits, classes, methods, moduleTrait.selfType, sql)
    }
  }
}
...
trait ScalanParsers {
  ...
  def methodDef(md: DefDef, isElem: Boolean = false) = {
    ...
    val optSql = md.rhs match {
      case call:Apply if call.fun.symbol.name == "sql" =>
        Some(call.args(0).asInstanceOf[Literal].value.stringValue)
      case _ => None
    }
    val optElem = if (isElem) Some(()) else None
    SMethodDef(md.name, tpeArgs, args, tpeRes, isImplicit, optOverloadId, optExternal, optSql, optElem)
  }
