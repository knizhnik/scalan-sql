...
trait ScalanCodegen extends ScalanParsers with SqlCompiler { ctx: EntityManagement =>
  ...
  class EntityFileGenerator(module: SEntityModuleDef, config: CodegenConfig) {
    ...
    def getTraitAbs = {
      val sqlSchema = if (module.sql.isEmpty) "" else generateSchema(module.sql)

      val sqlQueries = module.methods.filter(m => m.sql.isDefined).map(m => generateQuery(m)).mkString("\n\n")
