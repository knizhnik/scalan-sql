...
trait TablesAbs extends Scalan with Tables {
  ...
  implicit def proxyTableRecord(p: Rep[TableRecord]): TableRecord = {
    implicit val tag = weakTypeTag[TableRecord]
    proxyOps[TableRecord](p)(TagImplicits.typeTagToClassTag[TableRecord])
  }

  abstract class TableRecordElem[From, To <: TableRecord](iso: Iso[From, To])
    extends ViewElem[From, To](iso) {
    override def convert(x: Rep[Reifiable[_]]) = convertTableRecord(x.asRep[TableRecord])
    def convertTableRecord(x : Rep[TableRecord]): Rep[To]
  }
  ...
  class DetailElem(iso: Iso[DetailData, Detail])
    extends TableRecordElem[DetailData, Detail](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to Detail: missing fields List(id, desc, weight)")
  }
  ...
  // elem for concrete class
  class SupplierElem(iso: Iso[SupplierData, Supplier])
    extends TableRecordElem[SupplierData, Supplier](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to Supplier: missing fields List(id, company, address)")
  }
  ...
  // elem for concrete class
  class OrderElem(iso: Iso[OrderData, Order])
    extends TableRecordElem[OrderData, Order](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to Order: missing fields List(detail, supplier, amount, price, delivery)")
  }
  ...
  // elem for concrete class
  class LINEITEMElem(iso: Iso[LINEITEMData, LINEITEM])
    extends TableRecordElem[LINEITEMData, LINEITEM](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to LINEITEM: missing fields List(l_orderkey, l_partkey, l_suppkey, l_linenumber, l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate, l_commitdate, l_receiptdate, l_shipinstruct, l_shipmode, l_comment)")
  }
  ...
  // elem for concrete class
  class ORDERSElem(iso: Iso[ORDERSData, ORDERS])
    extends TableRecordElem[ORDERSData, ORDERS](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to ORDERS: missing fields List(o_orderkey, o_custkey, o_orderstatus, o_totalprice, o_orderdate, o_orderpriority, o_clerk, o_shippriority, o_comment)")
  }
  ...
  // elem for concrete class
  class CUSTOMERElem(iso: Iso[CUSTOMERData, CUSTOMER])
    extends TableRecordElem[CUSTOMERData, CUSTOMER](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to CUSTOMER: missing fields List(c_custkey, c_name, c_address, c_nationkey, c_phone, c_acctbal, c_mktsegment, c_comment)")
  }
  ...
  class SUPPLIERElem(iso: Iso[SUPPLIERData, SUPPLIER])
    extends TableRecordElem[SUPPLIERData, SUPPLIER](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to SUPPLIER: missing fields List(s_suppkey, s_name, s_address, s_nationkey, s_phone, s_acctbal, s_comment)")
  }
  ...
  // elem for concrete class
  class PARTSUPPElem(iso: Iso[PARTSUPPData, PARTSUPP])
    extends TableRecordElem[PARTSUPPData, PARTSUPP](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to PARTSUPP: missing fields List(ps_partkey, ps_suppkey, ps_availqty, ps_supplycost, ps_comment)")
  }
  ...
  // elem for concrete class
  class REGIONElem(iso: Iso[REGIONData, REGION])
    extends TableRecordElem[REGIONData, REGION](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to REGION: missing fields List(r_regionkey, r_name, r_comment)")
  }
  ...
  // elem for concrete class
  class NATIONElem(iso: Iso[NATIONData, NATION])
    extends TableRecordElem[NATIONData, NATION](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to NATION: missing fields List(n_nationkey, n_name, n_regionkey, n_comment)")
  }
  ...
  // elem for concrete class
  class PARTElem(iso: Iso[PARTData, PART])
    extends TableRecordElem[PARTData, PART](iso) {
    def convertTableRecord(x: Rep[TableRecord]) = // Converter is not generated by meta
!!!("Cannot convert from TableRecord to PART: missing fields List(p_partkey, p_name, p_mfgr, p_brand, p_type, p_size, p_container, p_retailprice, p_comment)")
  }
