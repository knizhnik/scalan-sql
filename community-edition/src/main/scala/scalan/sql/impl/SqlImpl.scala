...
trait SqlAbs extends Scalan with Sql {
  ...
  implicit def proxyTable[R](p: Rep[Table[R]]): Table[R] = {
    implicit val tag = weakTypeTag[Table[R]]
  ...
  abstract class BaseTableElemAbs[R, From, To <: BaseTable[R]](iso: Iso[From, To])(implicit schema: Elem[R])
    extends TableElem[R, From, To](iso) {
    override def convert(x: Rep[Reifiable[_]]) = convertBaseTable(x.asRep[BaseTable[R]])
    def convertTable(x : Rep[Table[R]]): Rep[To] = convertBaseTable(x.asRep[BaseTable[R]])
    def convertBaseTable(x : Rep[BaseTable[R]]): Rep[To]
  }
  class BaseTableElem[R](iso: Iso[BaseTableData[R], BaseTable[R]])(implicit val schema: Elem[R])
    extends BaseTableElemAbs[R, BaseTableData[R], BaseTable[R]](iso) {
    def convertBaseTable(x: Rep[BaseTable[R]]) = BaseTable(x.tableName)
  }
  ...
  // elem for concrete class
  class UniqueIndexElem[K, R](iso: Iso[UniqueIndexData[K, R], UniqueIndex[K, R]])(implicit val schema: Elem[R], val index: Elem[K], val keyPath: String)
    extends BaseTableElemAbs[R, UniqueIndexData[K, R], UniqueIndex[K, R]](iso) {
    def convertBaseTable(x: Rep[BaseTable[R]]) = // Converter is not generated by meta
!!!("Cannot convert from BaseTable to UniqueIndex: missing fields List(table, map, getKey)")
  }
  ...
  // elem for concrete class
  class NonUniqueIndexElem[K, R](iso: Iso[NonUniqueIndexData[K, R], NonUniqueIndex[K, R]])(implicit val schema: Elem[R], val index: Elem[K], val keyPath: String)
    extends BaseTableElemAbs[R, NonUniqueIndexData[K, R], NonUniqueIndex[K, R]](iso) {
    def convertBaseTable(x: Rep[BaseTable[R]]) = // Converter is not generated by meta
!!!("Cannot convert from BaseTable to NonUniqueIndex: missing fields List(table, map, getKey)")
  }
  ...
  // elem for concrete class
  class ReadWriteTableElem[R](iso: Iso[ReadWriteTableData[R], ReadWriteTable[R]])(implicit val schema: Elem[R])
    extends BaseTableElemAbs[R, ReadWriteTableData[R], ReadWriteTable[R]](iso) {
    def convertBaseTable(x: Rep[BaseTable[R]]) = // Converter is not generated by meta
!!!("Cannot convert from BaseTable to ReadWriteTable: missing fields List(records)")
  }
  ...
  // elem for concrete class
  class ReadOnlyTableElem[R](iso: Iso[ReadOnlyTableData[R], ReadOnlyTable[R]])(implicit val schema: Elem[R])
    extends BaseTableElemAbs[R, ReadOnlyTableData[R], ReadOnlyTable[R]](iso) {
    def convertBaseTable(x: Rep[BaseTable[R]]) = // Converter is not generated by meta
!!!("Cannot convert from BaseTable to ReadOnlyTable: missing fields List(records)")
  }
  ...
  // elem for concrete class
  class PairTableElem[R1, R2](iso: Iso[PairTableData[R1, R2], PairTable[R1, R2]])(implicit val leftSchema: Elem[R1], val rightSchema: Elem[R2])
    extends BaseTableElemAbs[(R1,R2), PairTableData[R1, R2], PairTable[R1, R2]](iso) {
    def convertBaseTable(x: Rep[BaseTable[(R1,R2)]]) = // Converter is not generated by meta
!!!("Cannot convert from BaseTable to PairTable: missing fields List(left, right)")
  }
  ...
  // elem for concrete class
  class ShardedTableElem[R](iso: Iso[ShardedTableData[R], ShardedTable[R]])(implicit val schema: Elem[R], val shardKeyPath: String)
    extends BaseTableElemAbs[R, ShardedTableData[R], ShardedTable[R]](iso) {
    def convertBaseTable(x: Rep[BaseTable[R]]) = // Converter is not generated by meta
!!!("Cannot convert from BaseTable to ShardedTable: missing fields List(nShards, distrib, shards)")
  }
  ...
  // elem for concrete class
  class ShardedViewElem[R](iso: Iso[ShardedViewData[R], ShardedView[R]])(implicit val schema: Elem[R], val shardKeyPath: String)
    extends BaseTableElemAbs[R, ShardedViewData[R], ShardedView[R]](iso) {
    def convertBaseTable(x: Rep[BaseTable[R]]) = // Converter is not generated by meta
!!!("Cannot convert from BaseTable to ShardedView: missing fields List(nShards, view)")
  }
