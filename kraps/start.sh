...
#      ssh "${hosts[i % n_hosts]}" "cd /srv/remote/all-common/tpch/data ; ulimit -c unlimited ; export LD_LIBRARY_PATH=. ; ./tpch -cache -tmp ~ -inmem-threshold 10000 $i $n_nodes $nodes" > node$i.log 2>&1 &
	ssh "${hosts[i % n_hosts]}" "cd /srv/remote/all-common/tpch/data ; ulimit -c unlimited ; export LD_LIBRARY_PATH=. ; ./tpch -dir hdfs://strong:9000 -format parquet -tmp ~ -inmem-threshold 100000000 $i $n_nodes $nodes" > node$i.log 2>&1 &
done
wait
