...
namespace Q5
{
    ...
    void sum2(RevenueValue& out, RevenueValue const& in)
    {
        out.sum += in.sum;
        out.count += in.count;
    }
    ...
    struct Revenue
    {
        ...
        friend void print(Revenue const& r, FILE* out) { 
            printf("%s, %f, %ld", r.n_name, r.revenue.sum, r.revenue.count);
        }
    };

    void revenue(Revenue& out, Pair<Key<name_t>,RevenueValue> const& in)
    {
    ...
    {
        return a->revenue.sum > b->revenue.sum ? -1 : a->revenue.sum == b->revenue.sum ? 0 : 1;
    }
    ...
    { 
        return
            ...
            filter<asiaRegion>()->
            mapReduce<Key<name_t>,RevenueValue,map,sum2>(25)->
            project<Revenue,revenue>()->
            sort<byRevenue>(25);
    }    
}
